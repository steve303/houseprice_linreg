...
plotqq = function(models) {
  ...
  par(mfrow = c(1, len))
  for (mod in models) {
    qqnorm(resid(mod), main = paste("Normal Q-Q Plot"), col = "darkgrey")
    qqline(resid(mod), col = "dodgerblue", lwd = 2)
  }
}
...
Normal Q-Q Plots (poly_int_aic & int_mod)
```{r echo=FALSE}
plotqq(list(poly_int_aic, int_mod))
```

Shapiro-Wilk Test (on first 5000 data points due to limitations of the built-in 'shapiro.test' function)

|                      | poly_int_aic                                          | int_mod                                          |
|----------------------|-------------------------------------------------------|--------------------------------------------------|
| Shapiro-Wilk p-value | `r shapiro.test(resid(poly_int_aic)[0:5000])$p.value` | `r shapiro.test(resid(int_mod)[0:5000])$p.value` |

Fitted versus Residuals Plots

```{r echo=FALSE}
plotRes(list(poly_int_aic, int_mod))
```
...
|                       | poly_int_aic                     | int_mod                     |
|-----------------------|----------------------------------|-----------------------------|
| Breusch-Pagan p-value | `r bptest(poly_int_aic)$p.value` | `r bptest(int_mod)$p.value` |
...
log(price) transformation for poly_int_mod_red and int_mod:
```{r}
...
plotqq(list(poly_int_mod_red_log, int_mod_log))
plotRes(list(poly_int_mod_red_log, int_mod_log))
```

|                       | poly_int_aic_log                                                          | int_mod_log                 |
|-----------------------|---------------------------------------------------------------------------|-----------------------------|
| RMSE                  | `r sqrt(mean((house_trn$price - exp(fitted(poly_int_mod_red_log))) ^ 2))` | `r sqrt(mean((house_trn$price - exp(fitted(int_mod_log))) ^ 2))` |

We see that RMSE improves for both models with a response transformation. However, the normality and equal variance assumptions still appear to not hold. 

To explore what it would take to achieve a LINE model, we explored removing data points with high influence, which did improve our normality plots. 
```{r}
...
plotqq(list(poly_int_mod_red_log_cook, int_mod_log_cook))
```


However, there is no justification that exists for removing these influential data points, so we cannot use the model generated by doing so. 

Therefore, we ultimately choose the model with the lowest test RMSE, the full 2-way interaction model with a log response transformation: `int_mod_log = lm(log(price) ~ (.) * (.), data = house_trn)`
